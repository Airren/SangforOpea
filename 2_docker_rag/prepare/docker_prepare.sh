...
UBUNTU_VERSION_CODENAME="ubuntu.22.04~${UBUNTU_CODE_NAME}"

deb_path="../../docker_deb"
mkdir -p $deb_path

BASE_URL="https://download.docker.com/linux/ubuntu/dists/${UBUNTU_CODE_NAME}/pool/stable/${ARCH}/"

function download_package() {
  local package_name=$1
  local url=$2
  echo "Downloading: $package_name"
  if ! wget -q --show-progress "$url" -O ${deb_path}/$package_name; then
...
if [ ! -f docker_packages/${containerd_name} ]; then
  ...
  download_package $containerd_name ${BASE_URL}${containerd_name}
fi
...
if [ ! -f docker_packages/${docker_ce_name} ]; then
  ...
  download_package $docker_ce_name ${BASE_URL}${docker_ce_name}
fi
...
if [ ! -f docker_packages/${docker_ce_cli_name} ]; then
  ...
  download_package $docker_ce_cli_name ${BASE_URL}${docker_ce_cli_name}
fi
...
if [ ! -f docker_packages/${docker_buildx_name} ]; then
  ...
  download_package $docker_buildx_name ${BASE_URL}${docker_buildx_name}
fi
...
if [ ! -f docker_packages/${docker_compose_name} ]; then
  ...
  download_package $docker_compose_name ${BASE_URL}${docker_compose_name}
fi
